...
官方 tag release 已配置为 Developer ID 签名并提交 Apple notarization。如果你在本地自己构建，默认仍使用 ad hoc 签名，除非手动提供自己的签名身份。
...
GitHub Actions 会在 macOS runner 上构建发布产物。推送 `v0.1.0` 这类标签时，会用 Developer ID 证书签名 App，提交 Apple notarization，staple 公证票据，然后把 zip 格式的 `.app` 包发布到 GitHub Release。
