...
- 语言：可以在“跟随系统”、English、简体中文之间切换。切换后按提示重启即可生效。
- 打开辅助功能设置
- 开机启动
- 打开 GitHub
- 版本号：反馈问题时可以直接提供。
...
- 公开 release 提供 Apple Silicon Mac 使用的 `arm64` 下载包。Intel Mac 如有需要可以从源码自行构建，但以后不再发布 `x86_64` 安装包。
...
MacFocusFix 使用 SwiftPM 构建，目标系统为 macOS 14 或更新版本。公开 release 面向 Apple Silicon（`arm64`）构建。
